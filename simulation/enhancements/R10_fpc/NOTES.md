...
engine in `R/*.R` and reuses its building blocks; **edits nothing** in
`R/*.R`. All new logic lives in this folder (`fpc_helpers.R`). Outputs land in
`sim_output/arc_runs/R10_fpc/` and `results/` — the locked results
...
`R/estimators.R::.se_des(..., clustered = TRUE)` exactly — same Hájek
...
| file | role |
|------|------|
| `run.R`          | driver. Sources `R/*.R` + `fpc_helpers.R`; selects PART A/B by SLURM array index. |
...
SMOKE=1 \
  SIM_CODE="$PWD/codes" \
  FPC_RUN_DIR="$PWD/simulation/enhancements/R10_fpc" \
  SLURM_ARRAY_TASK_ID=1 SLURM_CPUS_PER_TASK=4 \
  Rscript simulation/enhancements/R10_fpc/run.R
```
...
Smoke writes `*_partA_SMOKE.rds` and `results/R10_fpc_partA_summary_SMOKE.csv`
...
`Rscript simulation/enhancements/R10_fpc/_selftest.R` (prints `SELFTEST OK`).
...
```bash
Rscript simulation/enhancements/R10_fpc/aggregate.R
```

Writes:
- `results/R10_fpc_partA_summary.csv` — FA & CF × {fpc, no-fpc} for L1/L2 on R1.
- `results/R10_fpc_fraction_sweep.csv` — CF coverage vs `J`/`f`/`N_pop`.
...
**Summary CSVs** in `results/` (columns follow the run_sim/aggregate
...
- Outputs are namespaced under `arc_runs/R10_fpc/` and `results/`; rerunning
