...
~1 helper file. No `R/*.R` is edited.

---

## How Q0 / g0 are extracted from the population (the crux)

Read from `R/dgp.R::make_population()`. The realized finite population
...
SMOKE=1 \
  ...
  Rscript simulation/enhancements/R04_nuisance_rate/run.R
```
...
SIM_CODE="$PWD/codes" \
  ...
  Rscript simulation/enhancements/R04_nuisance_rate/aggregate.R
```
...
- **Aggregated:** `results/R04_nuisance_rate_summary.csv` — the per-rung ×
...
- **Outputs are non-destructive:** everything lands under
  `sim_output/arc_runs/R04_nuisance_rate/` and `results/`; the locked
