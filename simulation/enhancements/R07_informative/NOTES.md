...
The canonical population (`R/dgp.R::make_population`) draws **three
...
| file | role |
|---|---|
| `dgp_infsamp.R` | **new DGP logic.** `attach_uS(pop, rho, seed)` builds the per-PSU driver; `draw_sample_infsamp(pop, sample_seed, rho, lambda, uS_tab)` does PPS-WOR stage-1 + canonical stage-2 and returns the same `obs` contract as `draw_sample`. Reuses `apply_L` from `R/dgp.R`. |
| `cf_arms.R` | the **two CF arms.** `run_cf_pair(obs, learners)` runs `Fully-Aware-CF-unwt` and `Fully-Aware-CF-wt`, reusing the building blocks exposed by `R/estimators.R`: `make_cf_folds`, `.sl`, `.eif_from_tmle`, `.se_des`. The only difference between arms is `weights = NULL` vs `weights = w[tr]` in the OOF `.sl()` calls. |
...
| `aggregate.R` | combines per-task RDS into `results/R07_informative_summary.csv` (matching `aggregate_sim.R` columns) and prints the **decision-rule readout**. |
...
**Non-destructive:** nothing in `R/*.R` is edited. Outputs go to
`sim_output/arc_runs/R07_informative/` (per-task RDS + manifests) and
`results/R07_informative_summary.csv` — none of the locked results are
touched.
...
export SMOKE=1 SIM_CODE=$PWD/codes DATA_ROOT=$PWD/sim_output \
       R07_DIR=$PWD/simulation/enhancements/R07_informative \
       R07_OUT=$PWD/sim_output/arc_runs/R07_informative \
       SLURM_ARRAY_TASK_ID=1 SLURM_CPUS_PER_TASK=4
Rscript simulation/enhancements/R07_informative/run.R
```
...
SIM_CODE=$PWD/codes R07_OUT=$PWD/sim_output/arc_runs/R07_informative \
  Rscript simulation/enhancements/R07_informative/aggregate.R
```
...
- **Aggregated:** `results/R07_informative_summary.csv` with columns
