...
Kang–Schafer transforms in `R/dgp.R::apply_L`. Under `complex`, the
...
- Outputs land in a **private** subtree and never clobber locked results:
  ...
  - summary CSV → `results/R01_simple_control_summary.csv`.

## Files
- `run.R` — the driver (sources the engine; loops model_type+rung; tags output).
- `aggregate.R` — combines per-task RDS → `results/R01_simple_control_summary.csv`
...
Rscript <repo-root>/simulation/enhancements/R01_simple_control/run.R
...
SMOKE=1 SIM_CODE="$PWD/codes" REPO_ROOT="$PWD" DATA_ROOT="$PWD/sim_output" \
  Rscript simulation/enhancements/R01_simple_control/run.R
```
...
SIM_CODE="$PWD/codes" REPO_ROOT="$PWD" DATA_ROOT="$PWD/sim_output" \
  Rscript simulation/enhancements/R01_simple_control/aggregate.R
```
...
```bash
sbatch simulation/enhancements/R01_simple_control/submit.slurm     # --array=1-60 (FULL)
```
...
SIM_CODE="$SLURM_SUBMIT_DIR/codes" REPO_ROOT="$SLURM_SUBMIT_DIR" \
  ...
  Rscript simulation/enhancements/R01_simple_control/aggregate.R
```
Pull `results/R01_simple_control_summary.csv` back to the repo.
...
- Summary: `results/R01_simple_control_summary.csv` with columns
