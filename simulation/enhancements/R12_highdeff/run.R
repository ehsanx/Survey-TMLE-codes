...
# Nothing in R/*.R is modified. Outputs go to a private folder and a
...
.this_dir <- tryCatch({
  ...
  if (length(f)) dirname(normalizePath(f)) else file.path(REPO_ROOT, "simulation/enhancements", RUN_ID)
}, error = function(e) file.path(REPO_ROOT, "simulation/enhancements", RUN_ID))
