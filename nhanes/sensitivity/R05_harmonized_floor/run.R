...
#
# Mirrors nhanes/nhanes_arc.R (split-repeat B + rich diagnostics) but swaps the
...
#
# NON-DESTRUCTIVE: sources R/estimators.R etc. read-only; writes ONLY to
#   nhanes/nhanes_output/arc_runs/R05/   (per-cell RDS)
#   results/R05_summary.csv          (aggregated, written by the aggregator)
...
# -- byte-identical to the helper in nhanes/nhanes_arc.R.
...
# IDENTICAL encoding to nhanes/nhanes_arc.R so the design matrix + W_cols match.
...
# SAME columns/shape as nhanes/nhanes_arc.R so aggregate_nhanes.R works as-is.
