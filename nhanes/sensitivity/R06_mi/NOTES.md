...
## Purpose
The locked NHANES analysis (`nhanes/02_impute.R`) imputes missing covariates
...
imputation, m=40**, for the **primary estimator** and combines the
...
- **Certified primary** = the 3-learner near-Donsker library
  ...
  `nhanes/03_run_estimators.R`** (the L2-equivalent rung). The reported
...
- We deliberately **do NOT MI the full L1–L4 ladder** — that would multiply the
  expensive deep rungs (L3/L4) by 10. Only the primary `LIB` is run.
...
- Only **sources** the canonical engine `R/estimators.R` (read-only). All new
...
- Edits **nothing** in `R/*.R` or `nhanes/*.R`.
...
- Outputs go to this run's own folders (see below); the locked
  `results/*` and `nhanes/results/*` are untouched.
...
```
SMOKE writes `results/R06_mi_summary_SMOKE.csv` and
`results/R06_mi_primary_SMOKE.csv` and per-example
...
- `R06_mi_primary.csv`  — the primary (Fully-Aware-CF) MI Table-2 column.
...
`results/nhanes_results_summary.csv`, the L2_smooth rung,
...
- **PROCEED** if, for the primary (Fully-Aware-CF):
...
## Dependencies
- Engine: `R/config.R` (seeds; sourced indirectly is not required — the
  driver sets its own paths but reuses SEED=20260607), `R/estimators.R`
  (`run_estimators` + building blocks).
- Data: `nhanes/analytic/E{1,2,3,4}_analytic.rds` (must exist; produced by the
  upstream `nhanes/01_*` build). NOT the `_imputed.rds` frames.
