...
make_population <- function(scenario      = c("standard", "R1"),
  ...
  truth <- .scoped_seed(pop_seed + 1L, {
    ...
    blip <- gh_expit(m1m, sigma2_Y, nGH) - gh_expit(m0m, sigma2_Y, nGH)
  ...
  structure(
    list(
      ...
      params = list(scenario = scenario, model_type = model_type, te_log_odds = te_log_odds,
                    H = H, J_per_stratum = J_per_stratum, M_per_psu = M_per_psu, N_pop = N_pop,
                    beta_strat = beta_strat, sigma2_C = sigma2_C, sigma2_Y = sigma2_Y,
                    sigma2_A = sigma2_A, alpha_g = alpha_g, p_treat_target = p_treat_target,
