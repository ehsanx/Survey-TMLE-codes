...
$env:SMOKE="1"; $env:SIM_CODE="<repo-root>/codes"; `
  ...
  Rscript <repo-root>/codes/arc_runs/R11_resampling_eff/run.R

# bash (ARC login node):
SMOKE=1 SIM_CODE="$PWD/codes" \
  R11_OUT="$PWD/sim_output/arc_runs/R11_resampling_eff" \
  Rscript codes/arc_runs/R11_resampling_eff/run.R

# then aggregate the smoke RDS into the CSV:
Rscript codes/arc_runs/R11_resampling_eff/aggregate.R
```
Inspect `results/arc/R11_resampling_eff_summary.csv` (and the printed table).
...
```
sbatch codes/arc_runs/R11_resampling_eff/submit.slurm
```
...
- Summary CSV: `results/arc/R11_resampling_eff_summary.csv` (+ `_combined.rds`).
...
writes to `results/arc/`. Feeds the Web Appendix D table comparing
