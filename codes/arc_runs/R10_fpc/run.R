...
#
# NON-DESTRUCTIVE enhancement run. SOURCES the canonical engine (codes/*.R) and
# REUSES its building blocks; writes only into this run's own output folders.
# Nothing in codes/*.R is modified.
#
...
#
# Engine contract reused (verified against codes/*.R, 2026-06-07):
...
#
# Env vars:
#   SIM_CODE              code dir (default 'codes')
...
RUN_DIR <- Sys.getenv("FPC_RUN_DIR",
                      file.path(REPO_ROOT, "codes", "arc_runs", "R10_fpc"))
