...
- The driver `run.R` **sources the canonical engine read-only**
  (`codes/config.R`, `dgp.R`, `estimators.R`, `diagnostics.R`, `learners.R`).
- All Design C behaviour is in **`design_c.R`** (this folder), which only
  **passes overridden arguments** to the canonical `make_population()` and
  ...
  `base_m`, `alpha_strat`. **No new scenario is added to `codes/dgp.R`; no
...
- Outputs go to **private** locations only; the locked headline results
  ...
  - summary CSV   → `results/arc/R12_highdeff_summary.csv`
...
| `aggregate.R` | combines per-task RDS → `results/arc/R12_highdeff_summary.csv` (columns match `aggregate_sim.R`). |
...
SMOKE=1 SIM_N_REPS=100 SLURM_CPUS_PER_TASK=6 \
  ...
  Rscript "<repo-root>/codes/arc_runs/R12_highdeff/run.R"
```
...
and writes `results/arc/R12_highdeff_summary_SMOKE.csv` plus
...
```bash
sbatch codes/arc_runs/R12_highdeff/submit.slurm
```
...
SIM_CODE="$PWD/codes" SIM_OUT="$PWD/sim_output/arc_runs/R12_highdeff" \
  ...
  Rscript codes/arc_runs/R12_highdeff/aggregate.R
```

Writes `results/arc/R12_highdeff_summary.csv` (one row per rung × method,
...
- All summary columns match `aggregate_sim.R`/`run_sim.R` so the existing
  figure/table code can consume `results/arc/R12_highdeff_summary.csv` directly.
