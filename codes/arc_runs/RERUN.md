...
**Only the run packages changed.** The canonical engine `codes/*.R` (config, dgp, estimators,
...
rm -rf codes/arc_runs Nhanes/arc_runs
```

| Copy this local folder | → to this ARC path |
|---|---|
| `<repo-root>\codes\arc_runs\`  | `/scratch/YOUR_ALLOCATION/GitHub/survey-tmle2/codes/arc_runs/` |
| `<repo-root>\Nhanes\arc_runs\` | `/scratch/YOUR_ALLOCATION/GitHub/survey-tmle2/Nhanes/arc_runs/` |

(A ready-to-drag snapshot of exactly these two folders is staged at `..\survey-tmle2-ARC-upload\`.)
...
sbatch codes/arc_runs/R01_simple_control/submit.slurm
...
- **Re-run only specific failed task ids** (optional) — `sbatch --array=<ids> codes/arc_runs/<RID>/submit.slurm`.
...
E:/GitHub/...`. Export the paths once, from the repo root on scratch:
...
Rscript codes/arc_runs/R03_isolation_2x2/aggregate.R
Rscript codes/arc_runs/R11_resampling_eff/aggregate.R
Rscript Nhanes/arc_runs/R05_harmonized_floor/aggregate_R05.R
# each re-run, after it completes -> results/arc/<RID>_summary.csv:
Rscript codes/arc_runs/<RID>/aggregate.R          # R01, R02, R04, R07, R10, R06
```

Then rsync `results/arc/`, `sim_output/arc_runs/`, and `Nhanes/nhanes_output/` back to your machine.
