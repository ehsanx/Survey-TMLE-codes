...
- `estimators_isolation.R` — the helper. Defines `run_isolation(obs, learners, g_floor=0.05, V_cf=5L)`
  ...
  Reuses the engine building blocks exposed by sourcing `codes/estimators.R`:
...
- `aggregate.R` — combines per-task RDS into `results/arc/R03_isolation_2x2_summary.csv`
  using the SAME coverage/se_ratio/MCSE math and column names as `codes/aggregate_sim.R`.
...
- Summary CSV: `results/arc/R03_isolation_2x2_summary.csv` (+ `_combined.rds`).
...
$env:R03_DIR="<repo-root>/codes/arc_runs/R03_isolation_2x2"
...
SMOKE=1 SIM_CODE=$PWD/codes REPO_ROOT=$PWD DATA_ROOT=$PWD/sim_output \
  R03_DIR=$PWD/codes/arc_runs/R03_isolation_2x2 \
  R03_OUT=$PWD/sim_output/arc_runs/R03_isolation_2x2 \
  SLURM_CPUS_PER_TASK=6 Rscript codes/arc_runs/R03_isolation_2x2/run.R
```
...
sbatch codes/arc_runs/R03_isolation_2x2/submit.slurm
```
...
`results/arc/R03_isolation_2x2_summary.csv`, 8 rows (4 arms x 2 rungs). Key cells:
