...
#
# Lives in THIS run folder. Reuses (does NOT edit) codes/dgp.R. It EXTENDS the
...
# ---------------------------------------------------------------------
# The canonical population (codes/dgp.R make_population) already carries, per
...
#               codes/dgp.R). Hence  S _||_ (A,Y) | C  is VIOLATED. The
...
# the .se_des/.sl building blocks from codes/estimators.R.
# =====================================================================

# null-coalescing (codes/dgp.R defines this too, but keep self-contained)
...
# Same output contract as codes/dgp.R draw_sample(): a data.frame `obs` with
...
draw_sample_infsamp <- function(population, sample_seed = NULL, model_type = NULL,
  ...
  # ---- RNG-scoped sampling (mirrors codes/dgp.R .scoped_seed behaviour) ----
  samp <- local({
    has_old <- exists(".Random.seed", envir = .GlobalEnv, inherits = FALSE)
    if (has_old) old <- get(".Random.seed", envir = .GlobalEnv)
