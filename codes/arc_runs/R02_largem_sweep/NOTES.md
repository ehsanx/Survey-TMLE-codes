...
- All new logic lives in `helpers.R` (this folder): `one_rep_m()` (forwards
  ...
  columns). The m knob is the engine's existing `base_m` arg — `codes/dgp.R` is
  untouched.
- Per-task RDS → `sim_output/arc_runs/R02_largem_sweep/`
  (ARC: `$DATA_ROOT/arc_runs/R02_largem_sweep/`). Manifests in a `manifest/`
  subdir. Summary CSV → `results/arc/R02_largem_sweep_summary.csv`. The locked
...
SMOKE=1 SLURM_ARRAY_TASK_ID=1 Rscript codes/arc_runs/R02_largem_sweep/run.R
```
...
Rscript <repo-root>/codes/arc_runs/R02_largem_sweep/run.R
```
...
SMOKE=1 SMOKE_RUNG=L4_aggressive SLURM_ARRAY_TASK_ID=1 \
  Rscript codes/arc_runs/R02_largem_sweep/run.R
```
...
```bash
export REPO_ROOT="$PWD" DATA_ROOT="$PWD/sim_output" SIM_CODE="$PWD/codes"
Rscript codes/arc_runs/R02_largem_sweep/aggregate.R
```

Writes:
- `results/arc/R02_largem_sweep_summary.csv` — one row per (m, rung, method)
...
- `results/arc/R02_largem_sweep_diag.csv` — per (m, rung) targeting/positivity
