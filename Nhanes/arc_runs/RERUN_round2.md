...
## ⚠ ENGINE CHANGED — sync `codes/estimators.R` to ARC before either re-run

`codes/estimators.R` was edited (the Fully-Aware-CV arm now fits its nuisances
...
- `git pull` on ARC (if `codes/estimators.R` is committed + pushed), **or**
- copying the single file `codes/estimators.R` to
  `/scratch/YOUR_ALLOCATION/GitHub/survey-tmle2/codes/estimators.R`.

Verify on ARC: `grep -n "w_cv <- if (is.null(inpop))" codes/estimators.R` (should hit).
...
```
Outputs: `results/arc/R06_mi_summary.csv` + `R06_mi_primary.csv` (overwritten),
...
(`Nhanes/R/aggregate_nhanes.R`), regenerate figures, and re-run the number audit
(`codes/verify_inline_numbers.R`). Only the CV column should move; CF/FA/PA/NA
shift at most by split-Monte-Carlo noise (b_split_sd ~ 0.001–0.006). The certified
