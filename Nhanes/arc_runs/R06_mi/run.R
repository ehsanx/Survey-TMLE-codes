...
# (Nhanes/R/02_impute.R uses mice m = 1) with MULTIPLE IMPUTATION m = 40 for the
# CERTIFIED PRIMARY estimator on ALL FOUR examples, and combine the per-imputation
...
# i.e. the SAME `LIB` as Nhanes/R/03_run_estimators.R (the L2-equivalent rung),
...
#
# The reported headline arm is Fully-Aware-CF (the certified primary estimator),
# but we pool ALL arms returned by run_estimators so the full Table-2 column is
# available with MI intervals.
#
# NON-DESTRUCTIVE: SOURCES the canonical engine (codes/estimators.R) read-only;
...
M     <- geti("MI_M",   if (SMOKE) 2L else 40L)   # certified headline run used m = 40
...
LIB <- c("SL.glm", "SL.earth", "SL.glmnet")   # certified primary (== 03_run_estimators.R)
...
run_example_mi <- function(ex_id) {
  ...
  # (2) run the certified primary on each completed dataset --------------------
  ...
  # console echo: the certified primary + a quick all-arm view ----------------
  prim <- pooled[pooled$method == "Fully-Aware-CF", ]
  cat(sprintf("  CERTIFIED PRIMARY (Fully-Aware-CF): b=%.4f  se=%.4f  CI=[%.4f, %.4f]  df=%.1f  fmi=%.3f  signif=%s\n",
...
# certified-primary-only headline CSV (the actual Table-2 MI column) ----------
...
cat("\n==== CERTIFIED PRIMARY (Fully-Aware-CF) MI Table-2 column ====\n")
