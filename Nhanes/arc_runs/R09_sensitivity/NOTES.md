...
re-runs of the **certified primary** NHANES estimator on **modified adjustment
sets**, plus an empirical **design-validity check**. Each sensitivity = the same
certified-primary pipeline run on a different covariate set, reported as the
...
near-Donsker library used by `Nhanes/R/03_run_estimators.R`). Headline arm =
...
- SOURCES the canonical engine `codes/estimators.R` **read-only** (exposes
  `run_estimators` + building blocks). Edits **nothing** in `codes/` or `Nhanes/R/`.
...
- Outputs go to this run's **own** folders and never clobber the locked headline
  results (`Nhanes/nhanes_output/results/`, `Nhanes/results/`).
...
After the smoke run, inspect `results/arc/R09_design_check_SMOKE.csv` and the
E4-no-BMI delta:
...
- `codes/config.R` (seeds; sourced transitively is not required — `run.R` sets
  its own seed), `codes/estimators.R` (the engine), `codes/learners.R` (only if
...
- `Nhanes/analytic/{E1,E2,E4}_imputed.rds` for the sensitivity cells and
  ...
  `Nhanes/R/01_build_analytic.R` → `02_impute.R`; already present).
