...
# (laptop-runnable, no SLURM needed) of the CERTIFIED PRIMARY estimator on
...
#       estimator run.
#
# Each sensitivity = THE CERTIFIED PRIMARY run on the modified adjustment set.
# CERTIFIED PRIMARY library == Nhanes/R/03_run_estimators.R:
...
#
# NON-DESTRUCTIVE: SOURCES the canonical engine (codes/estimators.R) read-only and
...
# certified primary (== 03_run_estimators.R); SMOKE uses GLM-only for speed
...
# =====================================================================
# (a)-(c): per-cell sensitivity = certified primary on the modified covs,
...
run_cell <- function(g) {
  ...
  # ATE/CI delta on the certified primary (and on Fully-Aware single-fit too) --
...
cat("\n==== ATE/CI DELTA (certified primary, Fully-Aware-CF) ====\n")
