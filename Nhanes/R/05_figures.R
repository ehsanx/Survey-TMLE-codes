# =====================================================================
# Nhanes/R/05_figures.R  —  clean forest plots of the five arms per example
#
...
# reference at 0, and the certified Fully-Aware-CF drawn as a filled marker.
...
# Writing/manuscript/images/fig2_nhanes_forest.{png,pdf}.
# =====================================================================
...
ms_dir  <- file.path(REPO_ROOT, "Writing", "manuscript", "images"); dir.create(ms_dir, recursive = TRUE, showWarnings = FALSE)
ids <- c("E1", "E2", "E3", "E4")

ARMS <- c("Non-Aware", "Partially-Aware", "Fully-Aware", "Fully-Aware-CV", "Fully-Aware-CF")
PCH  <- c(1, 2, 0, 5, 19)        # CF = filled circle (certified primary)

load_res <- function(id) {
  f <- file.path(res_dir, id, "estimators_all_arms.rds"); if (!file.exists(f)) return(NULL); readRDS(f)
}
...
draw_forest <- function(o, main = NULL) {
  ...
  for (i in seq_len(k)) {
    ...
    text(txt_x, yp[i], sprintf("%.3f (%.3f, %.3f)", rr$b[i], rr$lcl[i], rr$ucl[i]),
