# =====================================================================
# Nhanes/R/03_run_estimators.R  —  the five survey-TMLE arms per example
#
...
# Fully-Aware (single-fit), Fully-Aware-CV, Fully-Aware-CF (certified primary).
#
# Saves Nhanes/results/<id>/estimators_all_arms.rds and a combined CSV.
# Usage: Rscript Nhanes/R/03_run_estimators.R            (all four)
#        Rscript Nhanes/R/03_run_estimators.R E3         (one example)
# =====================================================================

suppressMessages({ library(SuperLearner); library(survey); library(surveyCV); library(tmle) })
REPO_ROOT <- Sys.getenv("REPO_ROOT", ".")
source(file.path(REPO_ROOT, "codes", "estimators.R"))
