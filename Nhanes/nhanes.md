...
1. **Only 3 single-fit arms.** `nhanes2` calls the `tmle` package directly and builds Non/Partially/
   Fully-Aware by hand; it does **not** call `codes/estimators.R::run_estimators()`, so there is **no
...
**Fully-Aware-CF as the certified primary** estimate. Reporting only CF would discard the contrast that
...
| **Fully-Aware-CF** ✅ | yes | design (Eq 8) | **PSU-level cross-fitted** SL | **certified primary** |
...
**Implementation note.** `run_estimators()` in `codes/estimators.R` currently hard-codes the covariate
...
Outputs that the manuscript consumes (Figure 2, Table 2) are copied to `Writing/manuscript/images/` and
...
- **Table (→ manuscript Table 2 for the lead):** estimate, SE, 95% CI, and design df by arm; certified CF
  headline.
...
All under `Nhanes/R/`, sourcing `codes/config.R` for paths and `codes/estimators.R` for the engine. The
...
**First refactor (blocking):** generalize `codes/estimators.R::run_estimators()` to accept an arbitrary
...
- ✅ **Refactor (blocking item) done & validated.** `codes/estimators.R::run_estimators()` now takes
...
- ✅ **Registry + downloader written.** `Nhanes/R/examples.R` (E1–E4 configs) and
  `Nhanes/R/00_download_cache.R` (download-if-missing → `Nhanes/raw/<TABLE>.rds` + `manifest.csv`,
